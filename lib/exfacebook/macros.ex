defmodule Exfacebook.Macros do
  ...
  defp define_api_call(function_name, arguments, batch: is_batchable) do
    quote do
      ...
      def handle_call({unquote(:"#{function_name}"), unquote_splicing(arguments)}, _from, state) do
        {:reply, apply(Exfacebook.Api, unquote(function_name), [unquote_splicing(arguments)]),
         state}
      end
      ...
      if unquote(is_batchable) do
        ...
        def handle_call(
              {unquote(:"#{function_name}"), api, unquote_splicing(arguments)},
              _from,
              state
            ) do
          {:reply,
  ...
  """
  defmacro define_api(function_name, method, arguments, options \\ [batch: true])
           when method in [:get, :post, :delete] do
    define_api_call(function_name, arguments, options)
  end
end
